...
spec_funcOR = do
    describe "funcOR tests:" $ do
        it "printableStack (evalState funcOR ([], Map.empty, Map.empty, [BOOL True, BOOL True])) returns \"[True]\"" $ do
            printableStack (evalState funcOR ([], Map.empty, Map.empty, [BOOL True, BOOL True])) `shouldBe` "[True]"
        it "printableStack (evalState funcOR ([], Map.empty, Map.empty, [BOOL False, BOOL False])) returns \"[False]\"" $ do
            printableStack (evalState funcOR ([], Map.empty, Map.empty, [BOOL False, BOOL False])) `shouldBe` "[False]"
        it "printableStack (evalState funcOR ([], Map.empty, Map.empty, [BOOL False, BOOL True])) returns \"[True]\"" $ do
            printableStack (evalState funcOR ([], Map.empty, Map.empty, [BOOL False, BOOL True])) `shouldBe` "[True]"
        it "printableStack (evalState funcOR ([], Map.empty, Map.empty, [BOOL False, INT 10])) returns \"[ExpectedBool]\"" $ do
            printableStack (evalState funcOR ([], Map.empty, Map.empty, [BOOL False, INT 10])) `shouldBe` "[ExpectedBool]"
        it "printableStack (evalState funcOR ([], Map.empty, Map.empty, [])) returns \"[InvalidParameterAmount]\"" $ do
            printableStack (evalState funcOR ([], Map.empty, Map.empty, [])) `shouldBe` "[InvalidParameterAmount]"

spec_funcNOT :: Spec
spec_funcNOT = do
    describe "funcNOT tests:" $ do
        it "printableStack (evalState funcNOT ([], Map.empty, Map.empty, [BOOL True])) returns \"[False]\"" $ do
            printableStack (evalState funcNOT ([], Map.empty, Map.empty, [BOOL True])) `shouldBe` "[False]"
        it "printableStack (evalState funcNOT ([], Map.empty, Map.empty, [BOOL False])) returns \"[True]\"" $ do
            printableStack (evalState funcNOT ([], Map.empty, Map.empty, [BOOL False])) `shouldBe` "[True]"
        it "printableStack (evalState funcNOT ([], Map.empty, Map.empty, [INT 10])) returns \"[ExpectedBool]\"" $ do
            printableStack (evalState funcNOT ([], Map.empty, Map.empty, [INT 10])) `shouldBe` "[ExpectedBool]"
        it "printableStack (evalState funcNOT ([], Map.empty, Map.empty, [])) returns \"[InvalidParameterAmount]\"" $ do
            printableStack (evalState funcNOT ([], Map.empty, Map.empty, [])) `shouldBe` "[InvalidParameterAmount]"
...
spec_funcLess = do
    describe "funcLess tests:" $ do
        it "printableStack (evalState funcLess ([], Map.empty, Map.empty, [INT 2, INT 1])) returns \"[True]\"" $ do
            printableStack (evalState funcLess ([], Map.empty, Map.empty, [INT 2, INT 1])) `shouldBe` "[True]"
        it "printableStack (evalState funcLess ([], Map.empty, Map.empty, [FLOAT 2.5, FLOAT 2.5])) returns \"[False]\"" $ do
            printableStack (evalState funcLess ([], Map.empty, Map.empty, [FLOAT 2.5, FLOAT 2.5])) `shouldBe` "[False]"
        it "printableStack (evalState funcLess ([], Map.empty, Map.empty, [FLOAT 2.5, INT 10])) returns \"[False]\"" $ do
            printableStack (evalState funcLess ([], Map.empty, Map.empty, [FLOAT 2.5, INT 10])) `shouldBe` "[False]"
        it "printableStack (evalState funcLess ([], Map.empty, Map.empty, [INT 2, FLOAT 1.5])) returns \"[True]\"" $ do
            printableStack (evalState funcLess ([], Map.empty, Map.empty, [INT 2, FLOAT 1.5])) `shouldBe` "[True]"
        it "printableStack (evalState funcLess ([], Map.empty, Map.empty, [INT 2])) returns \"[InvalidParameterAmount]\"" $ do
            printableStack (evalState funcLess ([], Map.empty, Map.empty, [INT 2])) `shouldBe` "[InvalidParameterAmount]"
        it "printableStack (evalState funcLess ([], Map.empty, Map.empty, [INT 2, BOOL True])) returns \"[ExpectedNumber]\"" $ do
            printableStack (evalState funcLess ([], Map.empty, Map.empty, [INT 2, BOOL True])) `shouldBe` "[ExpectedNumber]"

spec_funcGreater :: Spec
spec_funcGreater = do
    describe "funcGreater tests:" $ do
        it "printableStack (evalState funcGreater ([], Map.empty, Map.empty, [INT 2, INT 1])) returns \"[False]\"" $ do
            printableStack (evalState funcGreater ([], Map.empty, Map.empty, [INT 2, INT 1])) `shouldBe` "[False]"
        it "printableStack (evalState funcGreater ([], Map.empty, Map.empty, [FLOAT 2.5, FLOAT 2.5])) returns \"[False]\"" $ do
            printableStack (evalState funcGreater ([], Map.empty, Map.empty, [FLOAT 2.5, FLOAT 2.5])) `shouldBe` "[False]"
        it "printableStack (evalState funcGreater ([], Map.empty, Map.empty, [FLOAT 2.5, INT 10])) returns \"[True]\"" $ do
            printableStack (evalState funcGreater ([], Map.empty, Map.empty, [FLOAT 2.5, INT 10])) `shouldBe` "[True]"
        it "printableStack (evalState funcGreater ([], Map.empty, Map.empty, [INT 2, FLOAT 1.5])) returns \"[False]\"" $ do
            printableStack (evalState funcGreater ([], Map.empty, Map.empty, [INT 2, FLOAT 1.5])) `shouldBe` "[False]"
        it "printableStack (evalState funcGreater ([], Map.empty, Map.empty, [INT 2])) returns \"[InvalidParameterAmount]\"" $ do
            printableStack (evalState funcGreater ([], Map.empty, Map.empty, [INT 2])) `shouldBe` "[InvalidParameterAmount]"
...
spec_funcExec = do
    describe "funcExec tests:" $ do
        it "printableStack (evalState funcExec ([], Map.fromList [(\"0\", [INT 1, INT 2, INT 3, FUNC \"+\"])], Map.empty, [CODEBLOCK \"0\"])) returns \"[]\"" $ do
            printableStack (evalState funcExec ([], Map.fromList [("0", [INT 1, INT 2, INT 3, FUNC "+"])], Map.empty, [CODEBLOCK "0"])) `shouldBe` "[]"
...
spec_funcMap = do
    describe "funcMap tests:" $ do
        it "printableStack (evalState funcMap ([], Map.fromList [(\"0\", [INT 10, FUNC \"*\"]), (\"1\", [INT 3, INT 2, INT 1])], Map.empty, [CODEBLOCK \"0\", LIST \"1\"])) returns \"[[30, 20, 10]]\"" $ do
            printableStack (evalState funcMap ([], Map.fromList [("0", [INT 10, FUNC "*"]), ("1", [INT 3, INT 2, INT 1])], Map.empty, [CODEBLOCK "0", LIST "1"])) `shouldBe` "[[30, 20, 10]]"
        it "printableStack (evalState funcMap ([], Map.empty, Map.empty, [FLOAT 5.0, FLOAT 5.0])) returns \"[ExpectedList]\"" $ do
            printableStack (evalState funcMap ([], Map.empty, Map.empty, [FLOAT 5.0, FLOAT 5.0])) `shouldBe` "[ExpectedList]"
        it "printableStack (evalState funcMap ([], Map.fromList [(\"1\", [INT 3, INT 2, INT 1])], Map.empty, [FLOAT 5.0, LIST \"0\"])) returns \"[ExpectedCodeblock]\"" $ do
            printableStack (evalState funcMap ([], Map.fromList [("1", [INT 3, INT 2, INT 1])], Map.empty, [FLOAT 5.0, LIST "0"])) `shouldBe` "[ExpectedCodeblock]"
        it "printableStack (evalState funcMap ([], Map.empty, Map.empty, [])) returns \"[InvalidParameterAmount]\"" $ do
            printableStack (evalState funcMap ([], Map.empty, Map.empty, [])) `shouldBe` "[InvalidParameterAmount]"

spec_funcEach :: Spec
spec_funcEach = do
    describe "funcEach tests:" $ do
        it "printableStack (evalState funcEach ([], Map.fromList [(\"0\", [INT 10, FUNC \"*\"]), (\"1\", [INT 3, INT 2, INT 1])], Map.empty, [CODEBLOCK \"0\", LIST \"1\"])) returns \"[10, 20, 30]\"" $ do
            printableStack (evalState funcEach ([], Map.fromList [("0", [INT 10, FUNC "*"]), ("1", [INT 3, INT 2, INT 1])], Map.empty, [CODEBLOCK "0", LIST "1"])) `shouldBe` "[10, 20, 30]"
        it "printableStack (evalState funcEach ([], Map.empty, Map.empty, [FLOAT 5.0, FLOAT 5.0])) returns \"[ExpectedList]\"" $ do
            printableStack (evalState funcEach ([], Map.empty, Map.empty, [FLOAT 5.0, FLOAT 5.0])) `shouldBe` "[ExpectedList]"
        it "printableStack (evalState funcEach ([], Map.fromList [(\"1\", [INT 3, INT 2, INT 1])], Map.empty, [FLOAT 5.0, LIST \"0\"])) returns \"[ExpectedCodeblock]\"" $ do
